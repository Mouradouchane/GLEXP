...
bool dynamic_arr_t_construction_basics() {
  core::array<int> arr(10,allocator, memory_tag::event);  // Capacity 10, resize 5

  if (arr.elements_count() != 10) return false;
...
bool dynamic_arr_t_push_and_resize_trivial() {
  core::array<int> arr(2, allocator, memory_tag::event);  // Start capacity 2, resize by 2

  // Push 3 elements to force a resize
  arr.push(10);
  arr.push(20);
  if (arr.elements_count() != 2)
    return false;  // Capacity should still be 2 before next push

  arr.push(30);  // Forces a resize

  if (arr.elements_count() != 4) return false;     // New capacity should be 4
...
bool dynamic_arr_t_push_and_resize_nontrivial() {
  core::array<std::string> arr(1, 1);  // Start capacity 1, resize by 1

  arr.push("Test1");
  arr.push("Test2");  // Forces resize with std::string
  arr.push("Test3");  // Forces another resize

  if (arr.elements_count() != 3) return false;     // Capacity should be 3
  if (arr.get_push_index() != 3) return false;  // Logical size should be 3
...
bool dynamic_arr_t_copy_assignment_nontrivial() {
  core::array<std::string> original(1, 1);
  original.push("A");
  original.push("B");

  core::array<std::string> copy(1, 1);
  copy = original;  // Test the operator= we fixed

  if (copy.elements_count() != 2 || copy.get_push_index() != 2) return false;
...
bool dynamic_arr_t_construct_count_size_and_begin_end() {
    core::array<int> arr(10, 5); // Initial capacity 10, resize factor 5
    if (arr.elements_count() != 10) return false;
    if (arr.size() != sizeof(int) * 10) return false;
    if (arr.begin() == nullptr) return false;
...
bool dynamic_arr_t_push_and_auto_resize() {
    core::array<int> arr(2, 2); // Small initial capacity
    arr.push(10);
    arr.push(20);
    arr.push(30); // Should trigger resize from 2 to 4

    if (arr.elements_count() != 4) return false;
    if (arr[2] != 30) return false;
    return true;
}

bool dynamic_arr_t_nontrivial_push_pop() {
    core::array<std::string> arr(2, 2);
...
bool dynamic_arr_t_copy_assignment() {
    core::array<int> original(2, 2);
    original.push(10);

    core::array<int> copy(1, 1);
...
bool dynamic_arr_t_move_assignment() {
    core::array<int> source(2, 2);
    source.push(50);
    int* original_ptr = source.begin();

    core::array<int> destination(1, 1);
